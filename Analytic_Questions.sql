...
-- 29. Performance Rating Trends Over Time
SELECT 
...
-- 30. Years in Current Role vs Performance Ratings
...
-- 31. Department and Gender Distribution
...
-- 32. Age Distribution by Department
SELECT e.Department,
    AVG(e.Age) as AvgAge,
    MIN(e.Age) as MinAge,
    MAX(e.Age) as MaxAge
...
-- 33. Correlation between Training Opportunities Taken and Years at Company
SELECT 
    CASE 
        WHEN e.YearsAtCompany <= 2 THEN '0-2 years'
        WHEN e.YearsAtCompany <= 5 THEN '3-5 years'
...
-- 34. Job Role Progression (Assuming higher job levels indicate progression)
SELECT e.JobRole,
...
-- 35. Impact of Distance from Home on Overtime
SELECT 
    CASE 
        WHEN e.DistanceFromHome_KM <= 5 THEN '0-5 km'
        WHEN e.DistanceFromHome_KM <= 10 THEN '6-10 km'
...
-- 36. Relationship between Stock Option Level and Years at Company
SELECT e.StockOptionLevel,
    AVG(e.YearsAtCompany) as AvgYearsAtCompany,
...
-- 37. Impact of Education Field on Salary within Job Roles
...
-- 38. Relationship between Business Travel and Years Since Last Promotion
SELECT e.BusinessTravel,
...
-- 39. Correlation between Environment Satisfaction and Years with Current Manager
SELECT e.YearsWithCurrManager,
    AVG(CAST(pr.EnvironmentSatisfaction AS FLOAT)) as AvgEnvironmentSatisfaction
FROM Employee e
JOIN PerformanceRating pr ON e.EmployeeID = pr.EmployeeID
...
-- 40. Impact of Marital Status on Attrition Rates
SELECT e.MaritalStatus,
    COUNT(*) as TotalEmployees,
    SUM(CASE WHEN e.Attrition = 1 THEN 1 ELSE 0 END) as AttritionCount,
    CAST(SUM(CASE WHEN e.Attrition = 1 THEN 1 ELSE 0 END) AS FLOAT) / COUNT(*) as AttritionRate
...
-- 41. Relationship between Performance Rating and Years Since Last Promotion
SELECT 
    CASE 
        WHEN e.YearsSinceLastPromotion = 0 THEN 'This year'
        WHEN e.YearsSinceLastPromotion = 1 THEN '1 year'
...
-- 42. Department-wise Training Opportunities Utilization
SELECT e.Department,
...
-- 43. Impact of State on Various Metrics
SELECT e.State,
    AVG(e.Salary) as AvgSalary,
    AVG(CAST(pr.JobSatisfaction AS FLOAT)) as AvgJobSatisfaction,
    AVG(CAST(pr.ManagerRating AS FLOAT)) as AvgPerformanceRating,
...
-- 44. Relationship between Hire Date and Current Performance
SELECT 
...
-- 45. What is the distribution of employees across different education levels? 
...
-- 46. Analyze the relationship between years of experience:
...
-- 47. Investigate the impact of job satisfaction factors (EnvironmentSatisfaction, JobSatisfaction, RelationshipSatisfaction)
...
-- 48. Explore the relationship between training opportunities:
...
-- 49. Analyze the distribution of employees across different departments 
...
-- 50. Investigate the impact of work-life balance on job satisfaction and performance ratings.
...
-- 51. Examine the relationship between distance from home (DistanceFromHome_KM) and job satisfaction factors.
...
-- 52. Analyze the distribution of performance ratings across different managers (using ManagerRating and EmployeeID columns).
...
-- 53. Investigate the relationship between stock option level (StockOptionLevel) and employee attrition.
...
-- 54. Explore the correlation between overtime (Overtime column) and performance ratings.
...
-- 55. Analyze the distribution of employees across different ethnicities and genders,
...
-- 56. Investigate the relationship between business travel (BusinessTravel column) and job satisfaction factors.
...
-- 57. What is the average salary and performance rating for employees based on their age groups?
SELECT 
    CASE
...
-- 58. How does the average performance rating vary across different job roles and departments?
SELECT 
    e.JobRole,
    e.Department,
    AVG(p.SelfRating) AS AvgSelfRating,
...
-- 59. What is the relationship between the number of years an employee has been with the company 
...
-- 60. How does the average performance rating vary based on the level of education and job role?
SELECT e.EducationField, e.JobRole,
    AVG(p.SelfRating) AS AvgSelfRating,
    AVG(p.ManagerRating) AS AvgManagerRating
FROM Employee e
...
-- 61. What is the relationship between the number of training programs attended 
...
-- 62. How does the average salary and performance rating vary across different marital statuses and gender?
...
-- 63. What is the relationship between the number of years at company an employee has worked for
